        ...
        <div class="col-md-12">
          ...
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Add Unit Address</h3>
            </div>
            <!-- /.card-header -->
            <form role="form" action="{{route('store-unit-address')}}" method="post">
              @csrf
              <div class="card-body">
                <div class="form-group">
                  <label for="exampleSelectLocation">Location<span class="text-danger">*</span></label>
                  <select name="location" class="form-control select2" id="exampleSelectLocation">
                    <option value="">--Select--</option>
                    @if(count($building_location_arr)>0)
                      @foreach($building_location_arr as $bkey=>$bval)
                          <option value="{{ $bval->id }}" {{ old('location')==$bval->id ?"selected":""}}>{{ buildingLocationByLocationID($bval->id) }}</option>
                      @endforeach
                    @endif
                  </select>
                ...
                <div class="form-group">
                  ...
                  <input name="unit_number" type="text" class="form-control" id="exampleInputUnitNumber" placeholder="Enter unit number" value="{{ old('unit_number')}}">
              ...
              <div class="card-footer">
                ...
                <a href="{{ route('add-unit-address') }}" class="btn btn-info"> <i class="fas fa-redo-alt"></i> Refresh</a>
