            ...
            <div class="card-body">
              ...
              <div class="tab-content" id="custom-content-above-tabContent">
                <div class="tab-pane fade show active" id="custom-content-above-terminate" role="tabpanel" aria-labelledby="custom-content-above-messages-tab">
                  <form role="form" action="{{route('update-internet-termination')}}" method="post">
                    @csrf
                    <div class="card-body">
                              ...
                              <div class="form-group">
                                <label for="InputServiceTime">Time<span class="text-danger">*</span></label>
                                  <div class="input-group date" id="timepicker" data-target-input="nearest">
                                    <input type="text" name="service_time" value="{{ empty(old('service_time')) ? currentTime() : old('service_time') }}"  id="InputServiceTime" class="form-control datetimepicker-input" data-target="#timepicker"/>
                      ...
                      <fieldset style="border: 1px solid #ced4da; padding:0px 15px;">
                        ...
                        <div class="row">
                          <div class="col-md-2">
                            <div class="form-group">
                              <label for="InputPlan">Internet Plan</label>
                      ...
                      <fieldset style="border: 1px solid #ced4da; padding:0px 15px;">
                        ...
                        <div class="row">
                          <div class="col-md-3">
                            <div class="form-group">
                              <label for="InputRefund">Deposit<span class="text-danger">* (Enter 0 if invoice(s) clear with deposit)</span></label>
                                <div class="input-group">
                                  <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="fas fa-dollar-sign"></i></span>
                                  </div>
                                  <input name="refund_amount" type="text" class="form-control updateTotal" id="InputRefund" placeholder="deposit amount" value="{{ old('refund_amount') }}">
                                </div>
                    ...
                    <div class="card-footer">
                      ...
                      <a href="{{route('internet-terminate-service')}}" class="btn btn-primary"> <i class="fas fa-redo"></i> Refresh</a>
...
$( document ).ready(function() {  
  ...
  $(".fee").on("blur", function(){
    var deposit_fee = parseFloat($("#InputRefund").val());
    var due_amount = parseFloat($("#InputDueAmount").val());
    var sum = (deposit_fee > due_amount)? (deposit_fee - due_amount): (due_amount - deposit_fee);
    (deposit_fee>due_amount) ? $('#CompanyRefund').prop( "checked", true ) : $('#CustomerPay').prop( "checked", true );
    $(".fee").each(function(){
      if($(this).val() !=="")
...
{    
  ...
  $.ajax({
    url: '{{ route("get-customer-with-terminate-details") }}',
    type: 'post',
    ...
    success: function(response){
      ...
      if(response != null)
      {
        ...
        $('#InputRefund').val(response.deposit_fee);
        ...
        (parseFloat(response.deposit_fee)>parseFloat(response.due_amount))?$('#InputTotalAmount').val((parseFloat(response.deposit_fee)-parseFloat(response.due_amount)).toFixed(2)):$('#InputTotalAmount').val((parseFloat(response.due_amount)-parseFloat(response.deposit_fee)).toFixed(2));
        (parseFloat(response.deposit_fee)>parseFloat(response.due_amount))?$('#CompanyRefund').prop( "checked", true ):$('#CustomerPay').prop( "checked", true );
      }
      else
      {
