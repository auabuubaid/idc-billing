          ...
          <div class="card">
            <div class="card-header">
              <h3 class="col-md-12 card-title">
                <form role="form" action="{{route('internet-report')}}" method="post">
                  @csrf
                  <div class="row">
                    <div class="col-md-4">
              ...
              <table id="example1" class="table table-bordered table-striped" style="width:99%; text-align: center">
                <thead>                  
                  <tr>
                    ...
                    <th><span class="text-dark">{{ internetHistoryTypeAbbreviationToFullName('CP')['full'] }}</th>
                    <th><span class="text-dark">{{ internetHistoryTypeAbbreviationToFullName('CL')['full'] }}</th>
                    <th><span class="text-dark">{{ internetHistoryTypeAbbreviationToFullName('SS')['full'] }}</th>
                  ...
                  @if(is_array($reportArr) && count($reportArr)>0)
                    @foreach($reportArr as $rkey=>$rval)
                      <tr>
                        ...
                        <td>{{ $rval['TS'] }}</td>
                        <td>{{ $rval['MP'] }}</td>
                        <td style="font-weight:bold;" class="text-primary">{{ ($rval['NR']+$rval['CP']+$rval['CL']+$rval['SS']+$rval['RC']+$rval['TS']+$rval['MP']) }}</td>
                      </tr>
                    @endforeach
                  @endif 
                </tbody>
                <tfoot>                  
                  <tr>
                    <th style="font-weight:bold;"><span class="text-danger">Grand Total</span></th>
                    <th style="font-weight:bold;" class="text-primary"></th>
                    <th style="font-weight:bold;" class="text-primary"></th>
                    <th style="font-weight:bold;" class="text-primary"></th>
                    <th style="font-weight:bold;" class="text-primary"></th>
                    <th style="font-weight:bold;" class="text-primary"></th>
