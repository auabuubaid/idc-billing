      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div class="card card-primary">
            ...
            <!-- /.card-header -->
            <form role="form" action="{{route('update-exchange-rate')}}" method="post">
              @csrf
              <div class="card-body">                  
                <div class="row">
                  <div class="col-md-6">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="InputRate">Rate <span class="text-danger">*</span></label>
                        <input name="rate" type="text" class="form-control" id="InputRate" placeholder="Enter exchange rate" value="{{ $exchangeRateArr->rate }}">
                      @error('rate')<span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                  </div>                                        
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="InputExchangeMonthYear">Exchange Month Year</label>
                      <select name="exchange_month_year" class="form-control select2" id="InputExchangeMonthYear" disabled>
                        <option value="">--Select--</option>
                        @php $start = $month = strtotime(currentDate2()) @endphp  @php  $end = strtotime('2021-01-01') @endphp
                        @while($end < $month)
                          <option value="{{ date('Y-m-25', $month) }}"{{ $exchangeRateArr->monthly_date==date('Y-m-25', $month) ?"selected":""}}>{{ date('M-Y', $month), PHP_EOL }}</option>
              ...
              <input name="id" type="hidden" value="{{ $exchangeRateArr->id }}">
              <!-- /.card-body -->
              <div class="card-footer">
                <a href="{{route('exchange-rate')}}" class="btn btn-warning"> <i class="fas fa-backward"></i> Back</a>
                <a href="{{route('edit-exchange-rate',['id'=>$exchangeRateArr->id])}}" class="btn btn-info"> <i class="fas fa-redo-alt"></i> Refresh</a>
